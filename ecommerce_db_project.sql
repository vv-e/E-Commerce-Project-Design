...
CREATE TABLE Inventory (
    ...
    CONSTRAINT chk_inventory_thresh CHECK (restock_threshold >= 0)
);

-- -------------------------------------------------------------
--    CUSTOMER (subtype of User)
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
--    TRANSACTION
--    shipping_address is a snapshot; customer address may change.
-- -------------------------------------------------------------
CREATE TABLE `Transaction` (
    ...
    CONSTRAINT fk_transaction_customer FOREIGN KEY (customer_id) REFERENCES Customer(user_id),
    CONSTRAINT chk_transaction_amount  CHECK (total_amount >= 0)
);

-- -------------------------------------------------------------
--    TRANSACTION ITEM (weak entity)
--    Composite PK; unit_price is a purchase-time snapshot.
-- -------------------------------------------------------------
CREATE TABLE TransactionItem (
    ...
    CONSTRAINT pk_transaction_item   PRIMARY KEY (transaction_id, product_id),
    CONSTRAINT fk_ti_transaction     FOREIGN KEY (transaction_id) REFERENCES `Transaction`(transaction_id)
        ON DELETE CASCADE,
    CONSTRAINT fk_ti_product         FOREIGN KEY (product_id) REFERENCES Product(product_id),
    CONSTRAINT chk_ti_quantity       CHECK (quantity > 0),
    CONSTRAINT chk_ti_unit_price     CHECK (unit_price >= 0)
);

-- -------------------------------------------------------------
--    REVIEW (weak entity)
--    One review per (customer, product) pair.
-- -------------------------------------------------------------
CREATE TABLE Review (
    customer_id  INT             NOT NULL,
    product_id   INT             NOT NULL,
...
-- -------------------------------------------------------------
-- SAMPLE DATA
-- Seed data inlined so this file runs with: mysql -u root -p < ecommerce_db_project.sql
-- -------------------------------------------------------------
